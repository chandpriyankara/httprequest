...
public:
    ...
    Response send(const std::string& method,
                  const std::map<std::string, std::string>& url_parameters,
                  ...
                  const std::vector<std::string>& headers = {})
    {
        ...
        return send(method, url_parameters, body, headers);
    }
                  ...
                  const std::vector<std::string>& headers = {})
    {
        ...
        if (::connect(socket, addressInfo->ai_addr, static_cast<socklen_t>(addressInfo->ai_addrlen)) < 0)
            throw std::system_error(
                getLastError(), std::system_category(), "Failed to connect to " + domain + ":" + port);
        ...
        while (remaining > 0)
        {
            ...
            if (size < 0)
                throw std::system_error(
                    getLastError(), std::system_category(), "Failed to send data to " + domain + ":" + port);
        ...
        for (;;)
        {
            ...
            if (size < 0)
                throw std::system_error(
                    getLastError(), std::system_category(), "Failed to read data from " + domain + ":" + port);
            else if (size == 0)
                break; // disconnected
            ...
            if (!parsedHeaders)
            {
                ...
                {
                    const auto i =
                        std::search(responseData.begin(), responseData.end(), std::begin(crlf), std::end(crlf));
